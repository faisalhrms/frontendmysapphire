...
const ObjectiveRemarksModal = ({ isOpen, onClose, objective }) => {
    ...
    React.useEffect(() => {
        reset({ remarks: objective?.remarks || "" });
    }, [objective, reset]);

    const onSubmit = async (data) => {
        try {
            setIsLoading(true);
            await api.put(`/hrms/hr/objective/${objective.id}/remarks/`, { remarks: data.remarks });
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div className="bg-white rounded-lg shadow-lg w-full max-w-lg p-6">
                <h3 className="text-lg font-semibold mb-4">{objective?.remarks ? 'Edit' : 'Add'} Remarks</h3>
...
export default ObjectiveRemarksModal;
