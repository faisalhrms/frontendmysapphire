...
const ConfirmReprocessModal = ({closeModal, onConfirm, message}) => {
  const handleClose = useCallback(() => {
    const modal = document.getElementById("confirmReprocessModal");
      ...
      <div className="hs-overlay-open:mt-7 ti-modal-box mt-0 ease-out h-[calc(100%-3.5rem)] min-h-[calc(100%-3.5rem)] flex items-center">
        <div className="max-h-full mx-auto overflow-hidden ti-modal-content">
          <div className="ti-modal-header">
            <h6 className="modal-title">Confirm Reprocess</h6>
            <button
              type="button"
          ...
          <div className="ti-modal-body">
            <p>{message || "Are you sure you want to reprocess the selected files?"}</p>
          </div>
          <div className="ti-modal-footer">
            <button onClick={handleConfirm} type="button" className="ti-btn ti-btn-danger">
              Yes
            </button>
            <button onClick={handleClose} type="button" className="ti-btn ti-btn-secondary">
              No
            </button>
          </div>
        </div>
