...
import { uploadCorrectionUpdates } from "@modules/beirholm-bi/services/CorrectionRulesService.js";
import Notify from "@helpers/toastNotifications.js";
import FormButton from "@components/form/FormButton.jsx";

const UploadCorrectionUpdatesModal = ({ title = "Upload Rule Updates", closeModal, onUploaded }) => {
  const [file, setFile] = useState(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const modalId = "uploadCorrectionUpdatesModal";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = await uploadCorrectionUpdates(file);
      Notify.success("Updates queued. Re-applying rules in background.");
      onUploaded?.(payload);
      handleClose();
    } catch (err) {
      if (!err?.response) Notify.error("Failed to upload updates");
    } finally {
  ...
  return (
    <div
      id="uploadCorrectionUpdatesModal"
...
export default UploadCorrectionUpdatesModal;
