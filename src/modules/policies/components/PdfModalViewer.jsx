...
import { X, ZoomIn, ZoomOut, Maximize2, Minimize2, FileText, Camera, Loader2, ShieldAlert } from "lucide-react";
...
const PdfModalViewer = ({ isOpen, fileId, onClose }) => {
            ...
            renderToolbar: (Toolbar) => () => (
                <Toolbar>
                    {(slots) => {
                        ...
                        // Filter out unwanted keys explicitly
                        ...
                        // Render remaining valid toolbar components
                        return (
                            <>
                                {Object.entries(rest).map(([key, Component]) => {
                                    if (typeof Component === 'function') {
                                        try {
                                            ...
                                            console.warn(`Error rendering toolbar component ${key}:`, error);
                                        }
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-80 z-50 flex justify-center items-center select-none">
            <div className={`bg-white rounded-lg shadow-2xl relative transition-all duration-300 flex flex-col ${
                    ...
                    <div className="flex items-center space-x-2">
                        {
                            ...
                            <div className="flex items-center space-x-1 bg-white rounded-lg border border-gray-200 p-1">
                                ...
                                <button onClick={handleResetZoom}
                                        className="px-2 py-1 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded min-w-[50px]"
                                        title="Reset Zoom">
                                    {zoom}%
                                </button>
                        ...
                        <button onClick={toggleFullscreen} className="p-2 hover:bg-gray-100 rounded-lg"
                            ...
                            {isFullscreen ? <Minimize2 className="h-4 w-4 text-gray-600"/> :
                ...
                <div className="flex-1 h-[calc(100%-73px-41px)] relative overflow-hidden bg-gray-100">
                    {loading ? (
                        ...
                        <div className="h-full w-full overflow-auto relative">
                            {!loading && (
                                <div className="absolute top-0 left-0 right-0 bottom-0 pointer-events-none opacity-10 text-6xl font-bold flex items-center justify-center text-gray-700 select-none z-[49]">
                                    CONFIDENTIAL
                                </div>
                            )}
                            {isImage ? (
