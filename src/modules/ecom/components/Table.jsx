        ...
        <table className="w-full table-fixed border-collapse">
          <thead className="bg-gray-200 dark:border-gray-700 dark:text-gray-200 text-gray-700 uppercase">
            <tr>
              ...
              <th
                colSpan="3"
                className="p-2 text-center border border-gray-400 dark:border-gray-700"
              >
                CY
              </th>
              <th
            ...
            <tr>
              {headers.map((header, index) => (
                <th
                  key={index}
                  className="p-3 border border-gray-300 dark:border-gray-700 text-center"
                >
                  {header.label}
                </th>
              ))}
            ...
            {loading ? (
             ...
             <td colSpan={5} className="text-center py-4">
              ...
              data.map((row, rowIndex) => (
                <tr
                  key={rowIndex}
                  className="border border-gray-300 dark:border-gray-700 transition hover:bg-gray-100"
                >
                  ...
                  <td className="border border-gray-400 dark:border-gray-700 text-center">
                    {formatNumber(row?.cy_order)}
                  </td>
                  <td className="border border-gray-400 dark:border-gray-700 text-center">
                    {formatNumber(row?.cy_qty)}
                  </td>
                  <td className="border border-gray-400 dark:border-gray-700 text-center">
                    {formatNumber(row?.cy_value)}
                  </td>
                  <td className="border border-gray-400 dark:border-gray-700 text-center">
                    {formatNumber(row?.ly_order)}
                  </td>
                  <td className="border border-gray-400 dark:border-gray-700 text-center">
                    {formatNumber(row?.ly_qty)}
                  </td>
                  <td className="border border-gray-400 dark:border-gray-700 text-center">
                    {formatNumber(row?.ly_value)}
                  </td>
                  <td
                    className={`border border-gray-400 dark:border-gray-700 text-center ${
                      row?.yoy_order < 0 ? "text-red-600" : ""
                    }`}
                  >
