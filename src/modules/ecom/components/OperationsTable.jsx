...
const OperationsTable = ({ title, columns, data }) => {
        ...
        <div className="p-2">
            ...
            <div className="overflow-x-auto">
                <table className="w-full border-collapse border border-gray-400">
                    <thead>
                    <tr className="text-black dark:text-gray-200 border-gray-400 dark:bg-gray-200">
                        {columns.map((col, index) => (
                            <th key={index} className="border border-gray-200 bg-gray-200 dark:bg-bodybg px-6 py-3 text-left">
                                {col}
                            </th>
                        ))}
                    </tr>
                    ...
                    {data.length === 0 ? (
                        <tr>
                            <td colSpan={columns.length} className="text-left border-gray-200 p-4">
                                No Data Available
                            </td>
                        </tr>
                    ) : (
                        data.map((row, rowIndex) => (
                            <tr key={rowIndex} className="hover:bg-gray-200 text-left">
