                            ...
                            <>
                                {data.map((row, index) => (
                                    <tr key={index} className="border hover:bg-gray-50 dark:text-gray-200 dark:bg-bodybg">
                                        <td className="border p-2 text-start dark:text-gray-200 dark:bg-bodybg">{row.group}</td>
                                        <td className="border p-2 text-left dark:text-gray-200 dark:bg-bodybg">{row.site}</td>
                                        <td className="border p-2 text-right dark:text-gray-200 dark:bg-bodybg">{formatNumberWithCommas(row.activations)}</td>
                                        <td className="border p-2 text-right dark:text-gray-200 dark:bg-bodybg">{formatNumberWithCommas(row.orders)}</td>
                                        <td className="border p-2 text-right dark:text-gray-200 dark:bg-bodybg">{formatNumberWithCommas(row.merchandiseTotal)}</td>
                                        <td className="border p-2 text-right dark:text-gray-200 dark:bg-bodybg">{row.avgPerUsage}</td>
                                        <td className="border p-2 text-right dark:text-gray-200 dark:bg-bodybg">{row.avgPerOrder}</td>
                                        <td className="border p-2 text-right dark:text-gray-200 dark:bg-bodybg">
                                            {row.orders && row.total_qty
                                                ? formatNumberWithCommas(row.total_qty / row.orders, 2)
                                                : "0"}
                                        </td>
                                        <td className="border p-2 text-right dark:text-gray-200 dark:bg-bodybg">
                                            {row.conversion ? formatPercentage(row.conversion) : "0"}
                                        </td>
                                    </tr>
                                ))}
                                ...
                                <tr className="border font-semibold bg-[#949eb7] dark:text-gray-200">
                                    <td className="border p-2 text-start">Total</td>
                                    <td className="border p-2 text-left"></td>
                                    <td className="border p-2 text-right">{formatNumberWithCommas(totals.activations)}</td>
                                    <td className="border p-2 text-right">{formatNumberWithCommas(totals.orders)}</td>
                                    <td className="border p-2 text-right">{formatNumberWithCommas(totals.merchandiseTotal)}</td>
                                    <td className="border p-2 text-right">
                                        {totals.activations && totals.merchandiseTotal
                                            ? formatNumberWithCommas(totals.merchandiseTotal / totals.activations, 2)
                                            : 0}
                                    </td>
                                    <td className="border p-2 text-right">
                                        {totals.orders && totals.merchandiseTotal
                                            ? formatNumberWithCommas(totals.merchandiseTotal / totals.orders, 2)
                                            : 0}
                                    </td>
                                    <td className="border p-2 text-right">
                                        {totals.orders && totals.total_qty
                                            ? formatNumberWithCommas(totals.total_qty / totals.orders, 2)
                                            : 0}
                                    </td>
                                    <td className="border p-2 text-right">
                                        {totals.activations && totals.orders
