...
const AnalysisConversionTable = ({ filters }) => {
    ...
    const totals = data.reduce(
        (acc, row) => {
            ...
            // merchandiseTotal comes as string with commas, remove commas to sum
            const merchValue = Number(row.merchandiseTotal.replace(/,/g, ""));
            acc.merchandiseTotal = (acc.merchandiseTotal || 0) + (isNaN(merchValue) ? 0 : merchValue);

            // avgPerUsage, avgPerOrder, itemsPerOrder can be float strings - sum them as numbers
            ...
            // For conversion, since it is a percentage string like "23.5%", remove '%' and sum
            const convValue = Number(row.conversion.replace("%", ""));
            acc.conversion = (acc.conversion || 0) + (isNaN(convValue) ? 0 : convValue);
                        ...
                        {data.length > 0 ? (
                            <>
                                ...
                                <tr className="border font-semibold bg-[#949eb7] dark:text-gray-200">
                                    ...
                                    <td className="border p-2 text-right">{formatNumber(totals.activations) ?? 0}</td>
                                    <td className="border p-2 text-right">{formatNumber(totals.orders) ?? 0}</td>
                                    <td className="border p-2 text-right">{formatNumber(totals.merchandiseTotal) ?? 0}</td>
