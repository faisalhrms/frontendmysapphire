                                ...
                                {data.map((row, rowIndex) => (
                                    <tr key={rowIndex} className="border hover:bg-gray-50 dark:hover:bg-gray-700">
                                        {headers.map((header, colIndex) => (
                                            ...
                                            >
                                                {header.accessor === "avg_items_per_order" && row.orders && row.total_qty
                                                    ? (row.total_qty / row.orders).toFixed(2)
                                                    : header.accessor === "source_group"
                                                        ? row[header.accessor]
                                                        : formatNumberWithCommas(row[header.accessor])}
                                            </td>
                                        ))}
                                    </tr>
                                ))}
                                ...
                                <tr className="border font-semibold bg-[#949eb7]">
                                    {headers.map((header, colIndex) => {
                                        ...
                                        if (header.accessor === "source_group") {
                                            ...
                                            displayValue =
                                                totals.orders && totals.merchandise_total
                                                    ? formatNumberWithCommas(totals.merchandise_total / totals.orders)
                                                    : 0;
                                        } else if (header.accessor === "avg_items_per_order") {
                                            displayValue =
                                                totals.orders && totals.total_qty
                                                    ? formatNumberWithCommas(totals.total_qty / totals.orders)
                                                    : 0;
                                        } else {
                                            displayValue = formatNumberWithCommas(totals[header.accessor]);
                                        }
