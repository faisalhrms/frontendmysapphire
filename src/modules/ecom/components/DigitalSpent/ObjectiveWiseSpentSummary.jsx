            ...
            {loading ? (
                ...
                <table className="min-w-full border border-gray-300 shadow-md text-sm">
                    ...
                    <tr className="bg-gray-200 text-black text-sm">
                        <th rowSpan="2" className="px-4 py-2 border font-bold">Name Origin</th>
                        <th colSpan="3" className="px-4 py-2 border font-bold">Last Day</th>
                        <th colSpan="3" className="px-4 py-2 border font-bold">MTD</th>
                    </tr>
                    <tr className="bg-gray-200 text-black text-sm">

                        <th className="px-4 py-2 border font-bold">GoogleAds</th>
                        <th className="px-4 py-2 border font-bold">MetaAds</th>
                        ...
                        <React.Fragment key={idx}>
                            ...
                            {region.subCategories.map((sub, subIdx) => (
                                <tr key={subIdx} className="bg-gray-100 ">
                                    ...
                                    <td className="px-4 py-2 border text-right font-bold">{sub.lastDay.googleAds.toLocaleString()}</td>
                                    <td className="px-4 py-2 border text-right font-bold">{sub.lastDay.metaAds.toLocaleString()}</td>
                                    <td className="px-4 py-2 border text-right font-bold">{sub.lastDay.total.toLocaleString()}</td>
                                    <td className="px-4 py-2 border text-right font-bold">{sub.mtd.googleAds.toLocaleString()}</td>
                                    <td className="px-4 py-2 border text-right font-bold">{sub.mtd.metaAds.toLocaleString()}</td>
                                    <td className="px-4 py-2 border text-right font-bold">{sub.mtd.total.toLocaleString()}</td>
                                </tr>
                            ))}
                        </React.Fragment>
                    ))}
