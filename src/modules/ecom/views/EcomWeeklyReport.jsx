...
const DEFAULT_TAB = "user_journey";
...
const EcomWeeklyReport = () => {
    ...
    const { data, isLoading, refetch } = useFetchWithFilters(
        activeTab === "user_journey"
            ? "/ecom/weekly-report/user-journey/weekly/"
            : activeTab === "source_based_performance"
                ? "/ecom/weekly-report/source-based-performance/"
                : activeTab === "landing_page_performance"
                    ? "/ecom/weekly-report/landing-page-performance/"
                    : activeTab === "hour_traffic_rate"
                        ? "/ecom/weekly-report/traffic-performance/"
                        : activeTab === "order_detail_from_cc"
                            ? "/ecom/weekly-report/order-detail-15minutes/"
                            : activeTab === "top_selling_article"
                                ? "/ecom/weekly-report/top-selling-articles-15minutes/"
                                : "",
        filters
    );
    ...
    return (
        <>
            <PageHeader
                currentpage="Website Metrics"
                activepage="Reports"
                mainpage="Ecom Weekly Report"
            />
                    ...
                    },
                    {
                        ...
                        content: (
                            <TopSellingProductsReport
                                filters={filters}
                                isActive={"top_selling_products" === activeTab}
                            />
                        ),
                    },
                    {
