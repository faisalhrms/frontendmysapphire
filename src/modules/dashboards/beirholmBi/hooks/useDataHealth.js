...
const normalize = raw => {
  ...
  if (Array.isArray(f.data_category))
    f.data_category = f.data_category.map(d => d?.value ?? d)
  if (Array.isArray(f.product_country))
    f.product_country = f.product_country.map(d => d?.value ?? d)
  return pick(f)
}
