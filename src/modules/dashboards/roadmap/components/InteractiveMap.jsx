...
export default function InteractiveMap({chain}) {
  ...
  useEffect(()=>{const iv=setInterval(()=>setAnimationOffset(p=>(p+1)%100),100);return()=>clearInterval(iv)},[])
  ...
  const points=basePoints.map(p=>{const catObj=cats.find(c=>c.name.toLowerCase()===p.name.toLowerCase());const cid=catObj?.id;return{...p,cid,icon:iconMap[p.name],units:grouped[cid]||[]}}).filter(p=>p.units.length>0)
  const positions={
    fiber:{left:-30,top:170},spinning:{left:250,top:-80},"yarn-dyeing":{left:540,top:50},weaving:{left:799,top:30},
    processing:{left:1050,top:290},wadding:{left:909,top:530},stitching:{left:650,top:530},accessories:{left:370,top:530},packaging:{left:100,top:530}
  }
  const markers={
    fiber:{x:70,y:335},spinning:{x:350,y:75},"yarn-dyeing":{x:640,y:190},weaving:{x:899,y:190},processing:{x:1000,y:320},
    wadding:{x:1000,y:455},stitching:{x:750,y:460},accessories:{x:470,y:460},packaging:{x:200,y:460}
  }
  const[expanded,setExpanded]=useState({})
  const toggle=(cid,uid)=>setExpanded(e=>({...e,[`${cid}:${uid}`]:!e[`${cid}:${uid}`]}))
  const[canvasData,setCanvasData]=useState({unitName:"",groups:{}})
  const openCanvas=(cid,uid,unitName)=>{const rows=rowsByCat[cid]||[];const row=rows.find(r=>r.unit.id===uid)||{};const certs=row.certificates||[];const groups=certs.reduce((acc,c)=>{const type=c.certificate.certificate_type.name.toUpperCase();if(!acc[type])acc[type]=[];acc[type].push(c);return acc},{});setCanvasData({unitName,groups});const el=document.getElementById("hs-overlay-right");if(el)window.HSOverlay.open(el)}
  const wrapperRef=useRef(null)
  const[scale,setScale]=useState(1)
  useLayoutEffect(()=>{const obs=new ResizeObserver(([entry])=>{if(!entry)return;const{width}=entry.contentRect;setScale(Math.min(width/1400,1))});if(wrapperRef.current)obs.observe(wrapperRef.current);return()=>obs.disconnect()},[])
  if(!points.length)return null
  return(
    <div ref={wrapperRef} className="w-full overflow-auto pt-24">
      <div className="relative" style={{width:1400,height:720,transform:`scale(${scale})`,transformOrigin:"top left",left:"60px"}}>
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 800 600">
          ...
          <path d="M-500 300 L-5 300 L-5 80 L220 80 L220 180 L650 180 L650 399 L450 399 L450 320 L310 320 L310 400 L-190 400" stroke="url(#asphaltPattern)" strokeWidth="48" fill="none" strokeLinejoin="round"/>
          <path d="M-500 300 L-5 300 L-5 80 L220 80 L220 180 L650 180 L650 399 L450 399 L450 320 L310 320 L310 400 L-190 400" stroke="#e8e8e8" strokeWidth="3" fill="none" strokeDasharray="30,20" strokeDashoffset={-animationOffset} strokeLinecap="round"/>
        </svg>
