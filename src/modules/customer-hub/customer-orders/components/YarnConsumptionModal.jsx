...
const YarnConsumptionModal = ({ open, onClose, item, totalMeters, onTotalMetersChange, widthInches, widthCm, onComputed }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-end md:items-center justify-center p-0 md:p-6" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      ...
      <div className="relative w-full md:max-w-4xl rounded-t-2xl md:rounded-2xl overflow-hidden shadow-2xl bg-white dark:bg-bodybg pointer-events-auto">
        <div className="px-5 py-4 flex items-center justify-between">
          ...
          <button type="button" className="ti-btn ti-btn-light !mb-0" onClick={onClose}>Close</button>
        </div>
        <div className="p-5 max-h-[80vh] overflow-y-auto">
          <YarnConsumptionCard item={item} totalMeters={totalMeters} onTotalMetersChange={onTotalMetersChange} widthInches={widthInches} widthCm={widthCm} onComputed={onComputed} />
        </div>
      </div>
    </div>,
    document.body
