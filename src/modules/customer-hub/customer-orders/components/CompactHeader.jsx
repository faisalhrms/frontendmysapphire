...
const CompactHeader = ({ msg }) => {
  ...
  const when = msg.received_at || msg.start_date || msg.created_at || new Date().toISOString()
  const isManual = msg.source === "manual"
  const subject = isManual ? "" : (msg.subject || "")
  ...
  const badge = isManual ? `${srcLabel(msg.source)} • ${statusLabel(msg.status)}` : null
  return (
      ...
      <div className="px-4 py-2.5 flex items-center gap-3">
        ...
        <div className="flex items-center justify-between gap-2 min-w-0 flex-1">
          ...
          {badge ? (
            ...
            <span className="px-3 py-1 text-[.82rem] font-semibold truncate">{subject}</span>
          ) : null}
        </div>
