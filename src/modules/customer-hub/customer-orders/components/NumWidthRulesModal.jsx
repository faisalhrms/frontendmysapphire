...
const NumWidthRulesModal = ({ isOpen, onClose, onSaved, loomTypeOptions = [] }) => {
  ...
  const { fields, append, remove, replace } = useFieldArray({ control, name: "narrowRows" })
  ...
  useEffect(() => { if (isOpen) loadRows() }, [isOpen])
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      const widerPayload = { band: "wider", loom_type: "", operator: "", width_in: null, panels: widerPanels }
      const widerTask = widerId ? updateNumWidthRule(widerId, widerPayload) : createNumWidthRule(widerPayload)
    ...
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:max-w-5xl bg-white dark:bg-[#151515] rounded-xl shadow-xl border dark:border-white/10">
        <div className="px-5 py-3 border-b dark:border-white/10 flex items-center justify-between">
          ...
          <button onClick={onClose} className="ti-btn ti-btn-light !mb-0 !py-1 !px-2 rounded-md" type="button">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="p-5 space-y-6 max-h-[75vh] overflow-y-auto">
          <div className="rounded-lg border dark:border-white/10 p-4 bg-slate-50/60 dark:bg-white/5">
            ...
            <div className="grid grid-cols-12 gap-3 items-end">
              <div className="col-span-12 sm:col-span-4">
                ...
                <div className="form-control w-full !rounded-sm border bg-gray-50 dark:bg-white/10 !h-10 flex items-center px-3">All</div>
              </div>
              <div className="col-span-12 sm:col-span-4">
              ...
              <div className="col-span-12 sm:col-span-4">
                <FormInput name="wider.panels" control={control} errors={{}} placeholder="Panels" type="number" className={ctrlClass} />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 rounded-lg border dark:border-white/10 p-4 bg-slate-50/60 dark:bg-white/5">
            ...
            <div className="hidden sm:grid grid-cols-12 gap-3 font-medium uppercase text-slate-500 dark:text-white/60 px-1">
              ...
              <div className="col-span-3">Panels</div>
            </div>

            {loading && <div className="text-xs text-slate-500 dark:text-white/60">Loading rules…</div>}

            {fields.map((field, idx) => (
              <div key={`${field?.id ?? "new"}-${idx}`} className="rounded-lg border dark:border-white/10 p-3 bg-white dark:bg-[#101010]">
        ...
        <div className="px-5 py-3 border-t dark:border-white/10 flex items-center justify-end gap-2">
          <button onClick={onClose} type="button" className="ti-btn ti-btn-light !mb-0">Cancel</button>
          <button
