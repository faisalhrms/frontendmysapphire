...
import { Target, RefreshCw, Activity, BarChart3, Check, X } from "lucide-react"
...
import { getAgreement, approveAgreement, rejectAgreement } from "@modules/customer-hub/customer-orders/services/AgreementService.js"
...
const AgreementApprovalDetail = () => {
  ...
  const handleApprove = async () => {
    if (!agreementId || savingAction) return
    setSavingAction(true)
    try {
      if (formRef.current?.saveDraft) await formRef.current.saveDraft()
      await approveAgreement(agreementId, remarks || "Approved")
      await refetch()
      if (agreementId) {
        const ag = await getAgreement(agreementId)
        setAgreement(ag)
  ...
  const handleReject = async () => {
    ...
    try {
      if (formRef.current?.saveDraft) await formRef.current.saveDraft()
      await rejectAgreement(agreementId, remarks || "Rejected")
      await refetch()
      if (agreementId) {
        const ag = await getAgreement(agreementId)
        setAgreement(ag)
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200 dark:border-gray-700 overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200/60 dark:border-gray-700/50">
                  ...
                  <div className="flex bg-slate-100 dark:bg-gray-700 rounded-lg p-1">
                    <button
                      className={`w-8 h-8 rounded-md ${viewType === "grid" ? "bg-white dark:bg-gray-600 text-blue-600 dark:text-blue-400 shadow-sm" : "text-slate-500 dark:text-gray-400"}`}
                      onClick={() => setViewType("grid")}
                    >
                      <BarChart3 className="w-4 h-4 mx-auto" />
                    </button>
                    <button
                      className={`w-8 h-8 rounded-md ${viewType === "list" ? "bg-white dark:bg-gray-600 text-blue-600 dark:text-blue-400 shadow-sm" : "text-slate-500 dark:text-gray-400"}`}
