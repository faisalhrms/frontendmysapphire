...
const FetchEmailsModal = ({ closeModal, config, fetchFn, onDone }) => {
  const [top, setTop] = useState(10)
  const [unreadOnly, setUnreadOnly] = useState(!!config?.use_unread_only)
  const [days, setDays] = useState(1)
  const [since, setSince] = useState("")
  const [isSubmitting, setIsSubmitting] = useState(false)
  const modalId = "fetchEmailsModal"
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = await fetchFn({
        username: config?.mailbox_email,
        top: Number(top) || 10,
        unread_only: unreadOnly,
        since: normalizeSince(since),
        days: since ? null : Number(days) || null,
      })
        ...
        <div className="max-h-full mx-auto overflow-hidden ti-modal-content">
          <form onSubmit={handleSubmit}>
            <div className="ti-modal-header">
              <h6 className="modal-title">Fetch Emails</h6>
              <button
                type="button"
              ...
              <div className="grid grid-cols-12 gap-4">
                <div className="col-span-12">
                  ...
                  <input
                    ...
                    readOnly
                    className="block w-full border border-gray-200 rounded-sm text-sm dark:border-white/10 bg-gray-50 dark:bg-black/20"
                  />
                </div>
                <div className="col-span-6">
                  ...
                  <input
                    ...
                    className="block w-full border border-gray-200 rounded-sm text-sm dark:border-white/10"
                  />
                </div>
                ...
                <div className="col-span-6">
                  ...
                  <input
                    ...
                    className="block w-full border border-gray-200 rounded-sm text-sm dark:border-white/10 disabled:opacity-60"
                  />
                  <p className="mt-1 text-[0.7rem] opacity-60">
                    Ignored if Since date is set
                  </p>
                </div>
                <div className="col-span-6">
                  <label className="block mb-1 text-sm font-medium">
...
export default FetchEmailsModal
