...
import { useCustomerHubMail } from "@modules/customer-hub/hooks/useMailData.js"
import { fileKind, resolveCidHtml, sanitizeHtml, truncateWords } from "@modules/customer-hub/services/MailAppUtils.js"
import { AttachmentIcon } from "@modules/customer-hub/components/AttachmentIcon.jsx"
    ...
    <Fragment>
      <div className="container-fluid">
        <div className="main-mail-container !p-2 gap-x-2 flex h-[calc(100vh-2rem)] min-h-0 overflow-hidden">
          <div className="total-mails border dark:border-defaultborder/10 flex flex-col w-[380px] min-w-[320px] min-h-0">
            <div className="!p-4 flex items-center justify-between border-b dark:border-defaultborder/10 !bg-blue relative">
              <div className="flex items-center gap-2">
                <Inbox size={18} className="text-white" />
                <h6 className="font-semibold mb-0 text-[1rem] text-white">All Mails</h6>
              </div>
                  ...
                  return (
                    <li key={t.thread_key} className="border-b dark:border-defaultborder/20">
                      <div className={`flex items-start ${isSelected ? "bg-light dark:bg-black/30" : "hover:bg-light/40 dark:hover:bg-white/5"}`}>
                        {hasMultiple ? (
                          <button
                            className={`shrink-0 mt-3 ms-2 me-2 w-7 h-7 grid place-items-center rounded-full transition-all ${isOpen ? "bg-primary/10 text-primary" : "bg-transparent text-[#8c9097] hover:bg-light/70 dark:hover:bg-white/10"}`}
                        ...
                        <button className="w-full text-left p-3" onClick={() => selectThread(t.thread_key)}>
                          <div className="flex items-start">
                            <Avatar full_name={t.last_from_name || t.last_from} size="md" parentClasses="profile-timeline-avatar me-2" />
                            <div className="flex-grow min-w-0">
                              <div className="mb-1 text-[0.75rem] space-x-2">
                                ...
                                <span className="ltr:float-right rtl:float-left text-[#8c9097] dark:text-white/50 font-normal text-[.6875rem]">
                                  {formatThreadTime(t.last_received_at)}
                                </span>
                              </div>
          ...
          <div className="mails-information border dark:border-defaultborder/10 text-defaulttextcolor text-defaultsize flex-1 flex flex-col min-h-0">
            {selectedMessage ? (
              <>
                <div className="shrink-0 p-6">
                  <div className="flex flex-wrap gap-2 items-center mb-4">
                    <Avatar full_name={selectedMessage.from_name || selectedMessage.from_address} size="md" parentClasses="profile-timeline-avatar me-2" />
                    <div className="flex-grow">
                      <h6 className="mb-0 font-semibold text-[1rem] dark:text-defaulttextcolor/70">{selectedMessage.from_name || selectedMessage.from_address}</h6>
                      <span className="text-[#8c9097] dark:text-white/50 text-[0.75rem]">{selectedMessage.from_address}</span>
                    </div>
                  </div>
                  <div className="sm:flex block items-center justify-between">
                    <p className="text-[1rem] font-semibold mb-0">{selectedMessage.subject}</p>
                    <span className="text-[0.75rem] text-[#8c9097] dark:text-white/50">{dayjs(selectedMessage.received_at).format("MMM-DD-YYYY,hh:mmA")}</span>
                  </div>
                </div>
                <div className="flex-1 min-h-0 overflow-y-auto px-6 pb-6 mt-2">
                  <div className="mb-6 prose max-w-none dark:prose-invert">
                  ...
                  {extractions.length > 0 && (
                    <div className="mt-2 mb-5">
                      <h6 className="mb-3 font-semibold text-[0.8rem]">Extracted Fields</h6>
                      <div className="list-group-item">
                        <div className="table-responsive task-table overflow-hidden transition-all duration-300 !rounded-l-none">
                          <table className="table whitespace-nowrap table-bordered min-w-full !border-l">
                            <thead>
                              <tr className="border-b border-defaultborder">
                                {Array.from(new Set(extractions.flatMap((e) => Object.keys(e.data || {})))).map((key) => (
                                  <th key={key} className="relative text-center">{key.replace(/_/g, " ")}</th>
                                ))}
                              </tr>
                            </thead>
                            <tbody>
                              {extractions.map((ext) => {
                                const keys = Array.from(new Set(extractions.flatMap((e) => Object.keys(e.data || {}))))
                                return (
                                  <tr key={ext.id} className="border-b border-defaultborder">
                                    {keys.map((k) => (
                                      <td key={k} className="text-center">{ext.data?.[k] ?? "-"}</td>
                                    ))}
                                  </tr>
                                )
                              })}
