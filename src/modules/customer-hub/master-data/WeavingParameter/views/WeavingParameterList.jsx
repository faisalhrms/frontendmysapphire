...
import {
    exportWeavingParameters, uploadWeavingParameters
...
const WeavingParameterList = () => {
  ...
  const columns = [
    ...
    { Header: "Weave", accessor: "weave", Cell: ({ value }) => toTitleCase(value || "N/A") },
    ...
    { Header: "Dyeing %", accessor: "dyeing_cost_percent" },
    { Header: "Packing/Yd", accessor: "packing_cost_per_yard" },
  ];

  const buttons = (
    <div className="flex items-center gap-2">
      <Link to={WEAVING_PARAMETER.CREATE.path} className="ti-btn ti-btn-primary-full !py-1 !px-2 !text-[0.75rem]"><i className="ri-add-line" /></Link>
      <button onClick={() => exportWeavingParameters()} className="ti-btn ti-btn-success-full !py-1 !px-2 !text-[0.75rem]"><i className="ri-download-2-line" /></button>
  ...
  return (
    <>
      <DataTable columns={columns} title="Weaving Parameters" apiUrl="customer-hub/weaving-params/datatable/" externalFilters={['tab']} ref={dataTableRef} buttons={buttons} />
      {open && (
        <UploadMasterModal
