            ...
            <div className="flex-1 overflow-x-auto overflow-y-auto border border-gray-400" style={{ maxHeight: '650px' }}>
                ...
                <table className="w-full text-base">
                    ...
                    <tr className="text-white bg-[#4d5875]" style={{ position: 'sticky', top: '39px', zIndex: 10 }}>
                        <th className="px-2  py-3 text-sm text-center font-semibold border">Date</th>
                        <th className="px-2 py-3 text-sm text-center font-semibold border">Visitors</th>
                        <th className="px-2 py-3 text-sm text-center font-semibold border">Orders</th>
                        <th className="px-2  py-3 text-sm text-center font-semibold border">Conversion</th>
                    </tr>
                    </thead>
                    <tbody>
                    {data2025.length > 0 ? (
                        data2025.map((row, index) => (
                            <tr
                                key={index}
                                className="text-sm bg-white dark:text-gray-200 dark:bg-bodybg whitespace-nowrap"
                            >
                                <td className="px-2 py-3 border">{row.report_date}</td>
                                <td className="px-2  py-3 text-center border">{row.visitors}</td>
                                <td className="px-2  py-3 text-center border">{row.orders}</td>
                                <td
                                    className={`px-2 py-3 text-center font-medium border ${getColorClass(row.conv_percentage)}`}
                                >
                    ...
                    >
                        <td className="px-4  py-3  text-sm font-bold border">Total</td>
                        <td className="px-4  py-3 text-center text-sm font-bold border">
                            {total2025.visitors}
                        </td>
                        <td className="px-4  py-3 text-center text-sm font-bold border">
                            {total2025.orders}
                ...
                <table className="w-full text-base">
                    ...
                    {data2024.length > 0 ? (
                        data2024.map((row, index) => (
                            ...
                            >
                                <td className="px-2  py-3 border">{row.report_date}</td>
                                <td className="px-2 py-3 font-medium border">{row.visitors}</td>
                                <td className="px-2 py-3 font-medium border">{row.orders}</td>
                                <td
                                    className={`px-2  py-3 font-medium border ${getColorClass(row.conv_percentage)}`}
                                >
                                    {row.conv_percentage.toFixed(2)}%
                                </td>
                            </tr>
                    ...
                    >
                        <td className="px-4  py-3 font-bold text-sm border">Total</td>
                        <td className="px-4  py-3 text-center text-sm font-bold border">
                            {total2024.visitors}
                        </td>
                        <td className="px-4  py-3 text-center text-sm font-bold border">
