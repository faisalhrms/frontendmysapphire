...
const ObjectiveApprovers = ({ approvers = [] }) => {
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="p-12 dark:text-gray-200 dark:bg-bodybg">
                <div className="flex flex-col items-center">
                    {approvers?.map((item, index) => (
                            <React.Fragment key={item.approver.id}>
                                <div
                                    className="bg-white rounded-2xl p-6 border border-slate-200 shadow-lg hover:shadow-xl transition-all duration-300 group cursor-pointer min-w-[280px] dark:text-gray-200 dark:bg-bodybg"
                                >
                                    <div className="flex items-center space-x-4 dark:text-gray-200 dark:bg-bodybg">
                                        ...
                                        <div className="flex-1">
                                            <h4 className="font-bold text-slate-900 text-lg dark:text-gray-200 dark:bg-bodybg">
                                                {item.approver.full_name}
                                            </h4>
                                            <p className="text-sm font-semibold text-indigo-600 dark:text-gray-200 dark:bg-bodybg">
                                                {item.approver.designation}
                                            </p>
                                            <p className="text-xs text-slate-500 dark:text-gray-200 dark:bg-bodybg">{item.approver.email}</p>
                                        </div>
                                    </div>
                                </div>
