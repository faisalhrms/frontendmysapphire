...
//         return offerings.filter((o) => (o?.courses?.title ?? "").toLowerCase().includes(q));
//     }, [offerings, search]);
//
//     return (
...
//                     {filteredOfferings.length} {filteredOfferings.length === 1 ? "course" : "courses"} available
//                   </span>
//                                 </div>
//                             )}
//                         </div>
...
//                                             style={{
//                                                 backgroundImage: "url('https://images.unsplash.com/photo-1561070791-2526d30994b5?w=800&q=80')"
//                                             }}
//                                         ></div>
//                                         <div className="absolute bottom-6 left-6 z-20 flex items-center gap-4">
//                                             <button className="w-16 h-16 rounded-2xl bg-white/20 backdrop-blur-md border-2 border-white/40 flex items-center justify-center hover:bg-white/30 transition-all group/play">
...
export default function CourseOffering() {
    ...
    const [enrollingId, setEnrollingId] = useState(null);
    const [search, setSearch] = useState("");
    const [offerings, setOfferings] = useState([]); // should always be array
    const [error, setError] = useState("");
    const [successMessage, setSuccessMessage] = useState("");

    const normalizeToArray = (payload) => {
        // supports: []  |  {results: []}  |  {data: []}  |  {}  |  null
    ...
    const fetchPublished = async (overrideSearch) => {
        ...
        try {
            ...
            const payload = res?.data?.data; // could be [] OR {} depending on backend
    ...
    const filteredOfferings = useMemo(() => {
        ...
        return list.filter((o) => {
            // support both shapes: o.course.title OR o.courses.title
            const title = (o?.course?.title || o?.courses?.title || "").toLowerCase();
            return title.includes(q);
        });
    }, [offerings, search]);

    const totalOfferings = Array.isArray(offerings) ? offerings.length : 0;
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        {filteredOfferings.map((offering) => {
                            ...
                            const title = offering?.course?.title || offering?.courses?.title || "—";
                            const slug = offering?.course?.slug || offering?.courses?.slug || "—";
                        ...
                        <span className="font-medium">
                          {formatDate(offering?.start_date)} - {formatDate(offering?.end_date)}
                                            ...
                                            <div className="text-sm font-mono text-slate-500">{slug}</div>
                                        </div>
