...
const DownloadSampleModel = ({closeModal, onDownload}) => {
  const {handleSubmit, control, formState: {errors, isSubmitting}, reset} = useForm({defaultValues: {business_unit: ""}});
  const handleClose = useCallback(() => {
    const modal = document.getElementById("downloadSampleModel");
  ...
  return (
    <div id="downloadSampleModel" data-hs-overlay-keyboard="false" className="hs-overlay ti-modal [--overlay-backdrop:static] backdrop-blur-[0.08rem]">
      <div className="hs-overlay-open:mt-7 ti-modal-box mt-0 ease-out h-[calc(100%-3.5rem)] min-h-[calc(100%-3.5rem)] flex items-center">
        <div className="max-h-full mx-auto overflow-hidden ti-modal-content">
          <form
            onSubmit={handleSubmit(async ({business_unit}) => {
              await onDownload(business_unit);
              handleClose();
            })}
          >
            <div className="ti-modal-header">
              <h6 className="modal-title">Download Sample</h6>
...
export default DownloadSampleModel;
