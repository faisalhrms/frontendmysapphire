...
const ChainList = () => {
  ...
  const columns = [
    ...
    {Header: 'Quality', accessor: 'quality.name'},
    {Header: 'Process', accessor: 'process_method.name'},
    {Header: 'Products', accessor: 'products', Cell: ({value}) => renderGrouped(value)},
  ...
  const buttons = (
    <>
      <div className="flex space-x-2">
        <Link to={CHAIN_DESIGNER.CREATE.path} className="hs-dropdown-toggle ti-btn ti-btn-primary-full !py-1 !px-2 !text-[0.75rem]">
        ...
        >
          <i className="ri-download-2-line font-semibold align-middle"></i>
        </button>
        <button
  ...
  return (
    <>
      ...
      <DataTable key={tableKey} columns={columns} title="RoadMap Chain" apiUrl="chain/datatable/" buttons={buttons} />
      {isUploadModalOpen && (
        <BulkUploadModel closeModal={closeUploadModal} refreshTable={refreshTable} />
      )}
      {isDownloadModalOpen && (
        <DownloadSampleModel
