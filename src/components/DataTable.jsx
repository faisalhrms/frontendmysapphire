            ...
            <nav aria-label="Page navigation" className="pagination-style-3">
                <ul className="ti-pagination mb-0 flex-wrap">
                    <li className={`page-item ${!canPreviousPage ? 'disabled' : ''}`}>
                        <button type="button" className="page-link" onClick={() => canPreviousPage && gotoPage(pageIndex - 1)}>Prev</button>
                    </li>

                    {startPage > 1 && (
                        <>
                            <li className="page-item">
                                <button type="button" className="page-link" onClick={() => gotoPage(0)}>1</button>
                            </li>
                    ...
                    {Array.from({ length: endPage - startPage + 1 }, (_, i) => (
                        <li key={i + startPage} className={`page-item ${page === i + startPage ? 'active' : ''}`}>
                            <button type="button" className="page-link" onClick={() => gotoPage(i + startPage - 1)}>{i + startPage}</button>
                        </li>
                    ))}
                    ...
                    {endPage < totalPages && (
                        <>
                            ...
                            <li className="page-item">
                                <button type="button" className="page-link" onClick={() => gotoPage(totalPages - 1)}>{totalPages}</button>
                            </li>
                        </>
                    )}

                    <li className={`page-item ${!canNextPage ? 'disabled' : ''}`}>
                        <button type="button" className="page-link" onClick={() => canNextPage && gotoPage(pageIndex + 1)}>Next</button>
                    </li>
                </ul>
            </nav>
        );
        ...
        <div className="box custom-box">
            ...
            <div className="box-body">
                <div className="flex items-center justify-between mb-4">
                    <select className="form-control form-control-sm border me-2 max-w-[120px]" value={size} onChange={handleSizeChange}>
                    ...
                    <div className="flex items-center gap-2">
                        <input type="search" onChange={handleSearch} placeholder="Search Here" className="form-control form-control-sm" />
                    </div>
                </div>
                {isLoading ? (
                    <LoadingSpinner />
